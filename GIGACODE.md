...
Аналитик — один раз, по всем требованиям. Дизайн/ревью — конвейер по journey (критик стартует
сразу за своим дизайнером, не ждёт волну), не lockstep-волны с барьером. Диспетчеризация и
обоснование — оркестраторская процедура, не агентская: `run.md`, Фаза 2.
...
Контракт целиком живёт там, где он исполняется — `.gigacode/agents/pytest-stub-writer.md` и
`.gigacode/agents/browser-test-writer.md` — и в процедуре оркестратора, `run.md` Фаза 2.5. **Не
дублируется здесь**: `pytest-stub-writer`/`browser-test-writer` не читают `GIGACODE.md` (не
входит в их спецификацию), и он загружается в сессию каждого агента цикла независимо от того,
нужен ли ему pytest — раньше 92 из 216 строк этого файла были такой копией, не читаемой третью
агентов, которым она никогда не адресована.

Коротко, для ориентации: после `PASS`/`NEEDS_HUMAN` по всем journey оркестратор один раз
спрашивает про генерацию pytest; по одному `pytest-stub-writer` на каждый `PASS`-journey,
`browser-test-writer` — только по `--selenium` и с заданным `base_url`. Имена модулей уникальны
на весь репозиторий (`<jid>`-суффикс — иначе `import file mismatch` при совместной сборке).
Числа в отчёте — только из фактического вывода `pytest`, никогда по памяти или шаблону.
