...
**Контракт** (все пути в `output/tests/<JOURNEY_ID>/`, `<jid>` — номер journey в нижнем
регистре: `J01-onboarding-and-first-play` → `j01`):

| Файл | Содержание |
|---|---|
| `output/tests/<ID>/conftest.py` | Фикстуры journey |
| `output/tests/<ID>/data_<jid>.py` | Типизированные константы из таблиц `## Тестовые данные` |
| `output/tests/<ID>/api_stub_<jid>.py` | Детерминированная эмуляция API |
| `output/tests/<ID>/test_<jid>.py` | Один класс на кейс, один `test_` на шаг |
| `output/tests/<ID>/test_browser_<jid>.py` | Браузерные тесты (только при `--selenium`) |
| `output/tests/<ID>/README.md` | Отчёт о генерации и прогоне |
| `pytest.ini` | Реестр маркеров — один на репозиторий, правится человеком |

**Имена модулей уникальны на весь репозиторий.** Pytest импортирует модули по базовому имени
файла, поэтому два journey с файлом `test_data.py` дают одно имя модуля на весь прогон: второй
journey получает чужие константы или падает на сборке. По отдельности каждый каталог при этом
зелёный — дефект виден только при запуске всех journey сразу. Отсюда суффикс `<jid>` у всех
модулей, кроме `conftest.py` (его pytest обрабатывает отдельным механизмом), и имя `data_`
вместо `test_data_` — файл констант не должен попадать в сборку по маске `test_*.py`.
