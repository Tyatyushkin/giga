...
## Пробелы и вопросы — с идентификаторами

```json
"gaps": [{"id": "G-03", "text": "…"}],
"clarifyingQuestions": [{"id": "Q-03", "text": "…"}]
```

Унаследованный из плана пробел берёт id плана; найденный вами — `G-<journey>-<n>`.
Из шага ссылайтесь по id, не по номеру: `«…требованиями не задано (G-03, Q-03)»`.

Позиционная нумерация всё ещё принимается, но ломается при вставке: сдвигается всё, что
после, а ссылки в шагах остаются старыми. Линтер ловит неразрешимую ссылку правилом
`ref-dangling` — в обеих формах.
