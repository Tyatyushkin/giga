...
- `--writers sequential|parallel` → как запускать писателей тестов в Фазе 2.5, когда
  PASS-journey несколько; пропускает вопрос. По умолчанию — `sequential`
- `--selenium-url <адрес>` → адрес Selenium-сервера для браузерных тестов; по умолчанию
  `http://localhost:4444`. Оркестратор передаёт его в `E2E_SELENIUM_URL`
...
`--writers`, `--selenium`, `--base-url`, `--selenium-url`.
