...
**Markdown is the only primary artefact.** JSON siblings are generated by the orchestrator
via `scripts/md_to_case_json.py` after you finish — do not write them. This is the W2
optimisation: it removes the ~25% output-token overhead of dual-writing.
...
- Markdown is authoritative; the orchestrator regenerates JSON via
  `scripts/md_to_case_json.py` after your work, so do not edit JSON by hand.
