...
Значение в переменную кладёт оркестратор: он принимает адрес флагом `--base-url <адрес>`
команды `/e2e:run` либо берёт его из требований и экспортирует в `E2E_BASE_URL` перед запуском
pytest. Правило «своего адреса у тебя нет» — выше, в разделе про два адреса; здесь только
механика передачи.
