...
| Константы и стаб | `output/tests/<JOURNEY_ID>/test_data.py` — уже написан `pytest-stub-writer` |

Константы переиспользуются, а не дублируются: браузерный тест берёт значения из того же
`test_data.py`, что и stub-тест.

## Выходные файлы

| Файл | Содержание |
|---|---|
| `output/tests/<JOURNEY_ID>/test_browser_<JOURNEY_ID>.py` | Браузерные тесты |
...
из `test_data.py`. В теле теста нет ни адресов, ни селекторов, ни данных предметной области.
...
5. **Шаг без наблюдаемого в интерфейсе признака не превращается в браузерный тест.** Помечай
   ...
   не называют.
6. **Никаких литералов.** Селекторы, пути и данные — из `test_data.py`.
